...
int main(void)
{
	...
	while (1)
	{
		...
		{
			rx_actual_length = 0;
			ux_device_class_cdc_acm_read_run(cdc_acm, (UCHAR *)UserRxBuffer, 4, &rx_actual_length);
			__NOP();
			if((rx_actual_length>0) && (rx_actual_length<65)){
				ULONG actual_length;
				UINT status = ux_device_class_cdc_acm_write_run(cdc_acm, (UCHAR *)UserRxBuffer, rx_actual_length, &actual_length);
				while(status == UX_STATE_WAIT){//wait for sending
					status =ux_device_class_cdc_acm_write_run(cdc_acm, UX_NULL, 0, &actual_length);
				}
			}
